...
void MHZ19_uart::writeCommand(uint8_t cmd[], uint8_t* response)
{
	...
	if (response != NULL)
	{
        ...
        while (hserial.available() > 0 && (unsigned char)hserial.peek() != 0xFF)
		{
			hserial.read();
			Serial.println("error: byte offset " + String(hserial.peek()));
		}
		...
		for ( int i = 0; i < MHZ19_uart::REQUEST_CNT; i++)
		{
		    Serial.print(String(response[i])+" ");
	    }
	    Serial.println("");
	}
...
int MHZ19_uart::getSerialData()
{
	...
	// parse
	if (buf[0] == 0xff && mhz19_checksum(buf) == buf[MHZ19_uart::RESPONSE_CNT-1])
	{
		//valid response
		if (buf[1] == 0x86)
		{
